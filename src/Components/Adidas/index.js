...
const Adidas = (props) => {
  ...
  const shirtTextures = ["/assets1/shirt10.jpeg"];
  const mainShirtImage = "/assets1/shirt5.png";
  ...
  const handleFileChange = (event) => {
    ...
    if (file && ['image/jpeg', 'image/png'].includes(file.type)) {
      ...
      if (fileName === 'female.png') {
        ...
        const customModel = {
          small: "/model/small1.glb",
          medium: "/model/medium1.glb",
          large: "/model/large1.glb",
        }[modelSize];
        setModelPath(customModel);
      } else {
        alert('⚠️ No personalized model for this image. Showing FitQuiz-based model.');
        // No change to modelPath — keep what FitQuiz already loaded
      }
    } else {
  ...
  return (
    <>
      <Dialog open={true} className="Adidas" maxWidth="md" fullWidth onClose={props.closeAdidas}>
        <input
          type="file"
          accept="image/*"
          ref={fileInputRef}
        ...
        <Button className='close_' onClick={props.closeAdidas}>
          <IoIosCloseCircle size={24} />
        </Button>

        <h2 className="mb-1 font-weight-bold px-3 pt-3">Blue Rewind Threads</h2>
            ...
            {showFitQuiz && (
              <FitQuiz
                ...
                onComplete={(finalSize) => {
                  ...
                  setModelPath(`/model/${finalSize}.glb`);
                  setShowModel(true);
                }}
              />
            )}
